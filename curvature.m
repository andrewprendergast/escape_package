...
swimbladder = [(trackingData(:,2)+trackingData(:,20))./2, (trackingData(:,3)+trackingData(:,21))./2];
headDirection = [swimbladder(:,1)-trackingData(:,2), swimbladder(:,2) - trackingData(:,3)];

% simple case: run tail angle for all 8 points

for i = 1:2:16
    ptCoord{(i+1)/2} = [trackingData(:,5+i), trackingData(:,5+i+1)]; % cell array, each column contains coordinates for each point
end

for i = 1:8
    ptDir{i} = [ptCoord{i}(:,1) - swimbladder(:,1), ptCoord{i}(:,2) - swimbladder(:,2)];
