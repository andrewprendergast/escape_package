...
%     hist([escapes.latency], cutoff-2)
    ...
    fPoints = figure();
    set(fPoints, 'Position', [1 1200 1200 1200]);

    for i = 1:nAnalyses

        s(i) = subplot(1,nAnalyses,i);
%         gscatter([escapes.genotype], [escapes.(incAnalyses{i})], expNum)
        scatter([escapes.genotype], [escapes.(incAnalyses{i})], 10, '.')
        title(incAnalyses{i})
        hold on
        ...
        errorbar(groups, indMeans{i}, indErr{i}, '.k', 'LineWidth', 1);
...
elseif doAverage == 1; % averaging within larvae
    ...
    fMeans = figure();
    set(fMeans, 'Position', [1 1200 1200 400]);
...
%         gscatter(perLarva{i,2}, perLarva{i,1}, perLarva{i,3})
        ...
        errorbar(groups, grpMeans{i}, grpErr{i}, '.k', 'LineWidth', 1);
